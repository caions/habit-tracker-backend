...
export class PgHabitCompDateRepository implements HabitCompletionDateRepositoryProtocol {
  ...
  async create(habitCompDate: HabitCompletionDate) {
    await pool.query('INSERT INTO habit_completion_dates(id, habit_id, completed_date) VALUES($1, $2, $3)',
      [habitCompDate.id, habitCompDate.habitId, habitCompDate.completedDate]);
    return habitCompDate
  };

  async list() {
    const result = await pool.query<PgHabitCompletionDate>('SELECT * FROM habit_completion_dates')
  ...
  async findByHabitIdAndDate(habitId: string, completedDate: string) {
    const [habit] = (await pool.query<PgHabitCompletionDate>('SELECT * FROM habit_completion_dates WHERE habit_id = $1 AND completed_date = $2', [habitId, completedDate])).rows;
    return {
      id: habit?.id,
  ...
  async delete(habitId: string, completedDate: string) {
    await pool.query('DELETE FROM habit_completion_dates WHERE habit_id = $1 AND completed_date = $2', [habitId, completedDate])
  }
}